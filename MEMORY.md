...
12. **Bay connectors → CLIK-Mate 1.50 dual row, 2×5** (owner's decision, 2026-09-05). The
    ...
    title-block rev bumped to 1.4; netlist (38/189/0) and ERC (82) unchanged. The owner
    ran Update PCB from Schematic at 11:25 on 2026-09-05 (commit a606ea7): J1–J8 are now
    the 2×5 footprints at the old positions and orientations; re-routing is still open.
13. **3D models** (2026-09-05, owner's request): KiCad 10 ships no 3D model for any
    CLIK-Mate connector or the NANO2 fuse, and the two custom footprints had none, so the
    3D viewer showed bare pads. `carrier.3dshapes/` now holds simplified VRML placeholders
    (correct envelopes and colours, built from boxes/prisms) generated by
    `tools/gen_3d_models.py`: 503148-1090 2×5 RA (11.8 × 8.75 × 9.15), 502494-0270 1×2 RA
    (8.0 × 6.95 body with the narrowed nose per the stock F.Fab, height ~6.5 estimated),
    XIAO ESP32C6 (flat; a socketed variant is kept) modelled from the owner's photos:
    rounded 21 × 17.8 board, 14 half-hole castellations, USB-C 8.94 wide overhanging the
    +X end to 12.05, R button on the pin-1 (−Y) side and B opposite with LEDs behind them,
    13 × 11.3 white shield starting 6.5 mm behind the USB edge, u.FL at the antenna-end
    pin-1 corner, chip antenna along the antenna end on the +Y half; MP1584EN module per
    the owner's photo (black 22.1 × 16.8 PCB, 6.3 mm shielded inductor, SOIC-8 with pins
    along X, 0805 ceramics, SOD-123, four plated corner holes, **no trimmer**); Littelfuse
    NANO2 451/453 (6.1 × 2.69 × 2.69). Referenced from carrier.pretty; J10 and F1 got
    project-local copies of the stock footprints that differ only in the model path and a
    descr note, and their Footprint fields now say `carrier:`. The PCB's embedded
    footprints were text-patched to reference the models (all 35 resolve). Preview:
    `pn5180_carrier_3d_preview.png`, `tools/render_3d_preview.py`. Vendor STEP files
    (Molex 503148-1090 / 502494-0270 from molex.com or Mouser's "STP File", Seeed's XIAO
    ESP32C6 STEP from its wiki) would replace the placeholders; not fetched — molex.com is
    unreachable from the tool network and a download needs the owner's OK.
14. **XIAO soldered flat, no sockets** (owner, 2026-09-05). New footprint
    `carrier:Seeed_XIAO_ESP32C6_SMD` (`tools/gen_xiao_smd_fp.py`): 14 SMD pads 1.5 × 2.5 at
    2.54 mm on the module edges (Y ±8.9, rows 17.8 mm apart), pin 1 at (+7.62, −8.9), the
...
|---|---|---|
| CLIK-Mate 2.00 RA 1×10 (retired 2026-09-05; off the PCB since a606ea7, still in the stale `gerbers/`) | Molex 502494-1070; stock KiCad footprint body 24.0 mm long | KiCad footprint exists; Molex listing |
...
| XIAO ESP32C6 mechanicals | 2×7 THT at 15.24 mm rows for sockets, castellations on the long edges at 2.54 mm (rows 17.8 mm apart, used flat since 2026-09-05), USB end = pins 1/14, 21 × 17.8 mm | Seeed OPL KiCad lib; owner's photos |
...
so pin 1 was at the bottom on J1–J4 and at the top on J5–J8 with the 1×10 parts. After the
2×5 swap (2026-09-05) it is the reverse: pin 1 at the top on J1–J4 and at the bottom on
J5–J8, signal-pad row inboard, nail pads outboard (pcbnew).
...
- Portrait board **62.5 × 106.5 mm** (edge-line centres X 108.0–170.5, Y 52.0–158.5; the
  ...
  479 track segments (112 of them on B.Cu, across 26 nets), 142 vias, 4 zones on
  2026-09-04; after the owner's 2026-09-05 footprint swap and the U5 swap: 452 segments
  (104 on B.Cu), 122 vias, 4 zones (saved unfilled — refill in the GUI).
...
- **2026-09-05 11:25: the owner swapped J1–J8 on the PCB to the 2×5 CLIK-Mate 503148-1090**
...
- The board as saved (owner's a606ea7 + the U5 script swap) has its GND zones **unfilled**
...
- KiCad VRML models: 1 unit = 0.1 inch (divide mm by 2.54); model +Y is footprint −Y (up
  on screen); after that Y mirror, reverse each face's winding or `kicad-cli pcb render`
  perspective views show the faces black while the top view still looks fine. Material
  diffuseColor is honoured. `${KIPRJMOD}` in model paths resolves only when a `.kicad_pro`
  sits beside the board, so preview scripts must copy the project file too. KiCad's DRC
  ignores missing 3D models; `pcb export step` lists them as "File not found".
