...
XIAO ESP32C6 soldered flat at the top centre of the board. It runs from the printer's 24 V supply through an MP1584EN buck
...
| `tools/` | `gen_clikmate_2x5_fp.py` (the 503148-1090 footprint), `gen_xiao_smd_fp.py` (the XIAO castellated footprint), `gen_3d_models.py` (the models above), `render_3d_preview.py` (top, isometric and close-up PNGs of the board via `kicad-cli pcb render`; run with KiCad's bundled Python) |
...
**Input stage (all on the carrier):** F1 1 A (NANO2 451/453, fast-acting — see open item 13) → D2 SMAJ28A TVS (clamps motor
...
with 502438 terminals on 22 AWG. The XIAO is soldered to the carrier, so bay and power
...
- **R6/R7 (33 Ω) in series with SCK and MOSI**, a few mm from the XIAO's SCK/MOSI pads and
...
```cpp
// Seeed XIAO ESP32C6 soldered to the carrier (Arduino-ESP32 core 3.x, board "XIAO_ESP32C6")
#define PIN_A0   D0
#define PIN_A1   D1
#define PIN_A2   D2
...
- U5 at the top centre, USB-C end at the top board edge (the rule is a 1–2 mm overhang;
  as placed the module edge sits 0.5 mm inside and only the USB-C shell is 1.05 mm proud),
  antenna end (opposite the USB) pointing inboard with
