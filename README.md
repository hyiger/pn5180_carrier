...
| `carrier.3dshapes/` | Simplified VRML 3D models (boxes with the right envelopes and colours) for the parts KiCad ships no model for: the 2×5 and 1×2 CLIK-Mate receptacles, the XIAO on its sockets, the MP1584EN module, the NANO2 fuse. Generated by `tools/gen_3d_models.py`; drop in vendor STEP files when you have them |
