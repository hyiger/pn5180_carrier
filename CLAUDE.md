...
| `carrier.pretty/`, `fp-lib-table` | Project-local footprints: XIAO 2×7 socket, MP1584EN module pads, CLIK-Mate 503148-1090 2×5 receptacle (generated by `tools/gen_clikmate_2x5_fp.py`, geometry in its `descr`), plus copies of the stock J10 (502494-0270) and F1 (NANO2) footprints that differ only in pointing at a project 3D model. All reference `${KIPRJMOD}/carrier.3dshapes/*.wrl` |
...
| `tools/` | `gen_clikmate_2x5_fp.py`, `gen_3d_models.py`, `render_3d_preview.py` (top/iso/close-up PNGs via `kicad-cli pcb render`, run with KiCad's Python). Re-run the generators rather than hand-editing their outputs |
...
12. **Finish the 2×5 connectors on the PCB.** The owner ran Update PCB from Schematic on
    ...
    text-patched into the embedded footprints afterwards. Still to do: re-route the eight
    columns and the power lanes (every track to the old pads dangles), refill zones, re-run
    DRC, regenerate `gerbers/`. Before fab, confirm the footprint's nail-pad position and
    pad order against Molex SD-503148-1090 (see MEMORY.md item 12).
