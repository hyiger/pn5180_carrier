...
~30 cm away over cables) share one SPI bus with a Seeed XIAO ESP32C6 soldered flat at the top
...
- `gen_kicad.py` is the script that originally generated the schematic, symbol library,
  ...
  itself only stamps rev 1.3, while the sheet title block and README are at 1.4 since the
  2026-09-05 connector change). The owner has since
...
| `carrier.3dshapes/` | Simplified VRML 3D models (generated by `tools/gen_3d_models.py`) for the parts KiCad ships no model for: 2×5 and 1×2 CLIK-Mate, XIAO (flat, plus a socketed variant), MP1584EN module (no trimmer), NANO2 fuse — six files. Replace with vendor STEP files when available |
...
- Power distribution plan (drawn for the 1×10 connectors; re-derive after the 2×5 swap,
  ...
  with pcbnew afterwards: 2.10–2.15 mm on J5–J8, 2.15–2.20 mm on J1–J4 (1×10 figures; with the 2×5 footprints the nail pads sit
  1.575–1.675 mm from the edge depending on the 0.05 mm column nudges, pcbnew
  2026-09-05). The lane fits and no further outline change is needed. This bullet is
...
- No copper on either layer under the XIAO's antenna end; the USB end overhangs the
  board edge by 1–2 mm (as placed, the module edge is 0.50 mm inside the board edge, the
  USB-C shell 1.05 mm proud of it, and the SMD footprint's courtyard, which includes the
  overhang, crosses the edge by 1.3 mm).
...
1. ~~Straighten the board outline.~~ Done 2026-09-04 23:25 (bottom-right corner set to
   ...
   52–158.5). The zones are currently saved **unfilled** (Update PCB from Schematic
   dropped the fills on 2026-09-05): refill in the GUI before the next DRC/export.
   `gerbers/` predates the fix.
2. Finish PCB routing per the power plan (pre-swap DRC of 2026-09-04: one open each on
   +3V3, /RST, MOSI, MISO, SCK; after the 2026-09-05 swaps see MEMORY.md "Check status
   after the 2×5 and flat-XIAO swaps": 78 opens with zones refilled). No further outline
   move is needed: the 2×5 nail pads sit 1.575–1.675 mm from the edge.
3. Add mounting holes; stitch the top pour and clear its orphaned GND pads (pre-swap:
   R2.2, R3.2, R4.2, U1.5, J1.9 on islands without a via — U1.5 is the 74HC138's /E2, so a
   floating one disables every chip select; post-swap DRC with refilled zones shows 4
   starved thermals RN1.5, U1.5, J7.9, J7.10 and 10 GND opens); then regenerate
   `gerbers/`.
...
11. The XIAO as placed does not overhang: its module edge is 0.50 mm inside the top edge
    (the SMD footprint's courtyard, which includes the USB-C overhang, crosses the edge by
    1.3 mm). Move U5 up ~1.5–2.5 mm for the agreed 1–2 mm USB overhang, or relax the
    rule; either way the two silk_edge_clearance warnings go.
...
14. **XIAO soldered flat** (owner, 2026-09-05): U5 uses `carrier:Seeed_XIAO_ESP32C6_SMD`
    ...
    pin numbering as before, so the netlist is unchanged. Found by the 2026-09-05
    verification pass, all pending: **U5 pin 4 (/EN) is open** — its route jogged to B.Cu
    into the old through-hole and the SMD pad has no back-side copper (add a via beside
    pad 4 or re-route on F.Cu); three shorts where the /EN and /RST tracks cross the new
    pad 5 (BUSY); the F.Cu tracks end at the old hole centres and overlap the new pads by
